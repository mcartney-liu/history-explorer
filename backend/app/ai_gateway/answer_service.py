...
from .config import GroundingTuningConfig  # ADR-0028 Temporal Gate enablement: tol below is PO-approved runtime tuning (2026-08-17, sweet-spot=75)
...
def _build_exploration_context(next_exploration) -> str:
    """Read-only formatting of the FROZEN next_exploration into a prompt
    segment (AI Historian §33/§34, fix F5).

    Pure consumption: does NOT call the planner, does NOT reorder or rank,
    does NOT invent candidates. Appended to the AI user prompt only when
    non-empty so the model can reference evidence-bound next steps without
    fabricating its own. The planner output is treated as a sealed,
    graph-validated artifact.
    """
    if not next_exploration:
        return ""
    lines = []
    for item in next_exploration:
        if isinstance(item, dict):
            gid = item.get("global_id") or ""
            rel = item.get("relationship") or ""
            reason = item.get("reason") or ""
        else:
            gid = getattr(item, "global_id", "") or ""
            rel = getattr(item, "relationship", "") or ""
            reason = getattr(item, "reason", "") or ""
        if not gid:
            continue
        line = "- relationship=%s | target=%s" % (rel, gid)
        if reason:
    ...
    return (
        ...
        "The following next-step exploration candidates are derived from the "
        "knowledge graph and already validated by the evidence layer (they are "
        "NOT generated by you):\n"
        + "\n".join(lines)
        + "\nIf your answer naturally leads to one of them, you may point it out "
        "as a 'further exploration direction'. Reference ONLY the items listed "
        "here; never invent your own.\n"
    )
...
def grounded_answer(
    ...
    builder = GroundingBuilder(knowledge_service, tuning=GroundingTuningConfig(tol=75))  # ADR-0028: PO-approved tol=75 (2026-08-17); catches cross-era bug, keeps adjacent dynasties
    ...
    user_prompt = (
        prompt_service.user_prompt(question, facts)
        + _CITATION_INSTRUCTION
        + _build_exploration_context(next_exploration)
    )
    ...
    validator = ResponseValidator(knowledge_service, tuning=GroundingTuningConfig(tol=75))  # ADR-0028: PO-approved tol=75 (2026-08-17); catches cross-era bug, keeps adjacent dynasties
