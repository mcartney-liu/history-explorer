...
describe('defaultImageSrc — entity_identity 回退', () => {
  it('entity_identity.<global_id> 回退到 /entity-logos/<global_id 冒号转下划线>.webp', () => {
    expect(defaultImageSrc('entity_identity.ancient_india:civ-maurya')).toBe(
      '/entity-logos/ancient_india_civ-maurya.webp',
    )
  })

  it('global_id 含多个冒号时全部转下划线', () => {
    expect(defaultImageSrc('entity_identity.a:b:c')).toBe('/entity-logos/a_b_c.webp')
  })
