...
describe('ProductIntro (M90.3, ex-DiscoverPage M44)', () => {
  ...
  it('renders one card per capability with an artwork band', () => {
    const html = renderToStaticMarkup(<ProductIntro />)
    const cards = html.match(/discover-intro-card/g) ?? []
    expect(cards.length).toBe(4)
    const arts = html.match(/discover-intro-art"(?![a-z])/g) ?? []
    expect(arts.length).toBe(4)
    // Each card carries its drop-in artwork img (photo owns the card)
    const imgs = html.match(/discover-intro-art-img/g) ?? []
    expect(imgs.length).toBe(4)
  })

  it('uses no emoji glyphs (P0-1 guard) and drops the old icon medallion', () => {
    const html = renderToStaticMarkup(<ProductIntro />)
    const emoji =
      /[\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE0F}]/u
    expect(emoji.test(html)).toBe(false)
    // The medallion (discover-intro-icon) was removed — the artwork itself
    // carries the label, per PO feedback (no card-level mini logo).
    expect(html).not.toContain('discover-intro-icon')
  })
})
