                ...
                <>
                  ...
                  <svg className="rip-timeline-svg" viewBox={`0 0 ${TL_W} ${tlSvg.tlH}`} width="100%" role="img" aria-label={t('rip.timelineViewAria')}>
                    <g transform={`translate(${tlPan},0) scale(${tlZoom},1)`}>
                      <line x1={TL_PAD} y1={TL_TOP - 8} x2={TL_W - TL_PAD} y2={TL_TOP - 8} style={{ stroke: 'var(--color-paper-300)' }} strokeWidth="1" />
                      {tlSvg.validBands.map((b, i) => {
                        ...
                        const ry = y + 4
                        const rh = TL_ROW - 12
                        const overlapFill = b.overlaps.length > 0 ? 'var(--color-truth-moderate)' : 'var(--color-status-info)'
                        return (
                          <g key={b.gid ?? b.name}>
                            <rect x={rx} y={ry} width={rw} height={rh} rx="2" style={{ fill: overlapFill }} opacity="0.85" />
                            <text x={rx + 4} y={ry + rh / 2 + 4} fontSize="11" style={{ fill: 'var(--color-ink-900)' }}>{b.name}</text>
                            <text x={rx + rw - 4} y={ry + rh / 2 + 4} fontSize="10" style={{ fill: 'var(--color-ink-900)' }} textAnchor="end">{`${formatYear(b.start as number)} – ${formatYear(b.end as number)}`}</text>
                          </g>
                        )
                      })}
                    </g>
                    <text x={TL_PAD} y={TL_TOP - 12} fontSize="10" style={{ fill: 'var(--color-ink-500)' }}>{formatYear(tlSvg.minYear)}</text>
                    <text x={TL_W - TL_PAD} y={TL_TOP - 12} fontSize="10" style={{ fill: 'var(--color-ink-500)' }} textAnchor="end">{formatYear(tlSvg.maxYear)}</text>
                  </svg>
                </>
              )}
