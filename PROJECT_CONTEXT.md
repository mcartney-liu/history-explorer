...
# 5. Current State (Runtime v0.13.0 / Project Release vM30.1 / M30-A)

Released (runtime): **v0.13.0** (M9-003, 2026-07-22). Latest project release: **vM30.1** (M30-A, 2026-07-26) — Frontend Provenance Exploration Panel: a pure-frontend additive layer that surfaces the M29.1 runtime provenance projection (`GET /provenance/{entity_id}`) inside the Entity Page. Added `frontend/src/data/provenanceApi.ts` (HTTP-only client; `ProvenanceDisabledError` raised on a 404, so a disabled projection degrades to a friendly panel instead of a hard error) and `frontend/src/components/ProvenancePanel.tsx` (5-state container+view — loading / success / empty / disabled / error; shows Source / Claim / Reference only; `AbortController` aborts the fetch on unmount). `EntityPage.tsx` mounts `<ProvenancePanel entityId={entity.id} />` right after the relationship cluster, passing the **local id** (the backend indexes provenance by local id per ADR-006, NOT the `global_id` — this contract fix is the key correctness change). No backend / data / schema / validation / registry change; runtime stays `0.13.0`; no new dependency / AI / LLM. Release commit: **81c2058**.

**Architecture state (M30-A):** Frontend provenance layer added — the Entity Page now renders an opt-in `ProvenancePanel` that consumes the existing M29.1 `GET /provenance/{entity_id}` read model (v1 + legacy). The panel is defensive: a 404 (projection disabled) shows a friendly "disabled" state rather than an error; provenance stays a derived read model surfaced read-only on the frontend. The `PROVENANCE_PROJECTION` flag (M29.1) still gates the backend projection.

**Deferred (M30-A):** AI ingestion pipeline.
