...
NODE="/c/Users/haizhi/.workbuddy/binaries/node/versions/22.22.2/node.exe"
VITE_JS="C:/Users/haizhi/WorkBuddy/2026-07-13-10-54-28/frontend/node_modules/vite/bin/vite.js"
...
echo "==> 启动后端 uvicorn :8001 (带 CORS)"
cd "$ROOT/backend"
env -u HTTP_PROXY -u HTTPS_PROXY CORS_ORIGINS="$CORS" "$PY" -m uvicorn app.main:app --port 8001 --host 127.0.0.1 > /tmp/uvicorn.log 2>&1 &
BACK_PID=$!

echo "==> 启动前端 vite :5174 (managed node 直调 vite.js，绕开 .bin 符号链接误转)"
cd "$ROOT/frontend"
env -u HTTP_PROXY -u HTTPS_PROXY VITE_API_BASE=http://localhost:8001 "$NODE" "$VITE_JS" --port 5174 --host 127.0.0.1 > /tmp/vite.log 2>&1 &
FE_PID=$!
