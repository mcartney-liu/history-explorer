...
- **Project Release: vM19** (2026-07-25) — Relationship Centrality & Pair Explorer (M19): pure-frontend additive visualization over the M17/M18 insight views — `relationshipUtils` adds `calculateRelationshipCentrality` (undirected degree count per global_id over the EXISTING relationship matrix, never invents edges) and `filterEdgesBetweenPair` (returns only existing edges whose endpoints are exactly A and B, either direction). `RelationshipInsightPanel` adds two `<details>` blocks — Relationship Centrality (entity / global_id / relationship count, labels via injected `nameByGlobalId`) and Pair Relationship Explorer (two view-only selects showing only existing edges between the selected pair, no causal words). `App.tsx` derives an `exploreNameByGlobalId` map (already-fetched metadata only) and passes it to the panel. Backend unchanged, runtime stays `0.13.0` (non-runtime release)
