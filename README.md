...
- **Project Release: vM18** (2026-07-25) — Relationship Insight Controls & Local Export (M18): pure-frontend deterministic controls and local-only export over the M17 insight views — `relationshipUtils` now also exposes `normalizeRelationshipFilter` / `filterRelationshipMatrixByType` / `sortRelationshipMatrixByCount` (matrix type-filter + count-sort with stable tie-break) and `sortTimelineBands` / `normalizeTimelineRange` (timeline band sorting, null bounds last, no fabricated dates). The panel gains matrix filter/sort and timeline sort controls (view-state only) plus a new `insightExport` module (`serializeInsightReport` / `buildPrintableInsight`) for a client-side Blob JSON download and a printable view — generated locally, never uploaded, no persistence. It VISUALIZES only — no causal inference, no inferred edges, no new KG semantics, no AI; backend unchanged, runtime stays `0.13.0` (non-runtime release)
