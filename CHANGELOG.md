...
## [vM76] - 2026-08-01 — Non-runtime release (M76 Domain Adapter Framework Hardening project release)

> **Non-runtime release**. `frontend/package.json` remains `[0.13.0]`. vM76 — **Domain Adapter Framework Hardening**: backend framework refactor under Runtime Freeze. Extracts ontology as a shared knowledge model (`backend/app/core/domain/ontology.py` frozen dataclass, `HISTORY_ONTOLOGY` singleton); M76-A decouples adapter creation from registration (`AdapterRegistry` + `DomainMetadata`); M76-B adds adapter lifecycle contract tests (`test_domain_adapter_contract.py` / `test_ontology_contract.py`, 216 lines). Runtime Kernel untouched (`citation_model` / `exploration_planner` / `response_validator` / `evidence_claim` / `exploration` / `dataset_validator` zero change); runtime stays 0.13.0; zero new dependency. Backend pytest green; `freeze-check` EXIT 0. Explicitly NOT a product feature — framework enablement for future multi-domain (M77). Release tag: `vM76`.

## [vM75] - 2026-08-01 — Non-runtime release (M75 Domain Adapter + Dataset Loading + Deterministic Acquisition Pipeline project release)

M75 Domain Adapter + Dataset Loading + Deterministic Acquisition Pipeline — a backend shell-layer closure under Runtime Freeze (approved via Architecture Freeze Gate): M75-A adds the Domain Adapter framework (pluggable `BaseDomainAdapter` + `AdapterRegistry` + single `DomainSchema`); M75-B activates the real `FileSourceLoader` pipeline (`build_dataset_provider()` unconditionally wires `FileSourceLoader`, reads `data/sources.json` → 43 SourceRecords); M75-C adds a deterministic knowledge Acquisition Pipeline (`SourceRecord` → `DomainSchema` alignment, no LLM / Agent / OCR); Patch-001 eager-registers `HistoryAdapter` on `import domain` (fixes a registration-lifecycle defect — `AdapterRegistry.get("history")` now resolves without any consumer). Runtime Kernel untouched (`citation_model` / `exploration_planner` / `response_validator` / `evidence_claim` / `exploration` / `dataset_validator` zero change); backend pytest **290 passed**; `freeze-check` EXIT 0; no new dependency / AI / LLM / schema / enum change. Release tag: `vM75`.

## [vM74] - 2026-08-01 — Non-runtime release (M74 AI Grounding Runtime project release)

> **Non-runtime release**. `frontend/package.json` remains `[0.13.0]`. vM74 — **AI Grounding Runtime + Evidence-based Exploration**: activates the M36-era `ai_gateway` (ADR-0003) under the **frozen Trust Boundary** (KG = fact layer, AI = interpretation layer, AI never a fact source — M74 AI Architecture & Product Freeze). **Phase 1 deterministic entry**: `topicResolver` fixes the QuickStart Chinese-question 400 (semantic resolution to package/entity, zero backend hits on miss); error classification now distinguishes `invalid-topic` from network failures. **Phase 2 Grounding Runtime** (PO Freeze Revision approved): local-id→global-id mapping, relationship-pair resolver, **ClaimGraph** as the single processing unit (lazy assembly, entity + pair claims unified), deterministic **Evidence Selection** (tier priority, source dedup, full audit records), **Evidence Validation** Trust Gate (claim exists AND source exists AND binding valid — else REJECT, never warn-and-continue). **M74-003 deterministic grounded `/ai/explain`**: Runtime OFF branch upgraded from placeholder to Phase2-pipeline-driven grounded output (`engine=deterministic`, `grounded=true`, 4-field contract answer/evidence/confidence/next_exploration); frontend touchpoints T1 Package exploration suggestions + T2 Entity relationship insight + T3 TrustDisplay, all behind `VITE_AI_SUGGESTIONS_ENABLED` (default OFF = M73 byte-identical). **M74-004 Exploration Experience**: backend **Exploration Planner** (P7 self-recommendation eliminated 50%→0, P2 visited-aware, deterministic reason from claim text, evidence-aware ranking), **JourneyTrail** visualization (consumes existing event stream only — no profiling, no new storage), **Evidence Card** (reason / claim_text / source_title / source_tier / confidence — all backend-produced, zero frontend fact assembly), and **Evaluation Baseline** (golden set 20 — entity 8 / relationship 7 / next 5; offline evaluator: Grounding 100% / Citation 100% / Hallucination 0 / Helpfulness proxy 100%). Explicitly NOT: LLM runtime (AI_GATEWAY_ENABLED=false), free-form chat, generated facts, scoring/ranking/personalization, KG writes. localStorage holds only anonymous local exploration-event cache — no accounts, no cloud sync, no user profiling. Invariants: ENTITY_TYPES=8 / RELATIONSHIP_TYPES=18 untouched; runtime 0.13.0; zero new runtime dependency. freeze-check EXIT 0 (guard 9/9); tsc EXIT 0; backend 290 pytest green; vitest 132 files / 1123 tests green; Playwright 12 cases green (default 10 + AI-flag 2); consistency 7/7; Evaluation baseline PASS.

## [vM73] - 2026-07-31 — Non-runtime release (M73 Alpha Hardening project release)

> **Non-runtime release**. `frontend/package.json` remains `[0.13.0]`. vM73 — **Alpha Hardening**: upgrades the capability-complete Pre-Alpha to a stranger-friendly, regression-guarded **Alpha**. **Architecture hardening** (`App.tsx` de-complexed — `useNavigationHistory` + `usePackageContext` hooks with dependency injection keep telemetry single-sourced; Package contract reserves `owner?/version?/sourcePackage?` for future M77 — `validatePackage` gains zero new checks; ADR-0010 API Evolution & Compatibility Strategy, design-only, no v2 implementation). **UI polish**: `labels.zh` patch — 42 entities across 3 datasets gain Chinese labels (36 new + 6 verified, en↔zh parity), `getEntityDisplayName` prefers labels when present; DS Lite components (`Badge` / `Tabs` / `EmptyState`) + 8-spot high-frequency consolidation; favicon.svg kills the `/favicon.ico` 404 console noise. **QA**: Playwright E2E infra (`@playwright/test` dev-only; 3 specs / 5 critical-path cases — main exploration chain / cross-civilization / edge cases); **Bug Sweep** (mobile 375px horizontal overflow fixed via additive media queries; WAI-ARIA tabs keyboard pattern; freeze-check working-tree parser fix); **Metrics Closure** — `view_source` aggregation added to `explorationMetrics.ts` (`SourceViewMetric`: totalViews / distinctSources / avgViewsPerSession / sessionsWithSourceViews / sourceUsageRate / topSources), **zero new telemetry**; `complete_package` remains deferred. Known Issues (registered, not fixed): F1 React StrictMode double-effect telemetry, F4 Playwright `fill` × controlled input (E2E-only). Explicitly NOT: LLM runtime, free-form Q&A, generated facts, scoring / ranking / personalization. localStorage holds only anonymous local exploration-event cache — no accounts, no cloud sync, no user profiling. Pure frontend — zero backend / schema / AI Gateway change, backend diff = 0. Invariants: ENTITY_TYPES=8 / RELATIONSHIP_TYPES=18 untouched; runtime 0.13.0; no new runtime dependency (dev-only `@playwright/test`). freeze-check EXIT 0 (guard tests 9/9); tsc EXIT 0; vitest 126 files / 1078 tests green; Playwright E2E 5/5 green; consistency 7/7.

---

## [vM72] - 2026-07-31 — Non-runtime release (M71 + M72 combined project release)

> **Non-runtime release** (M71 + M72 combined). `frontend/package.json` remains `[0.13.0]`. vM72 — **Exploration Validation → Exploration Quality Improvement**: official Exploration Package library now **four civilizations** (China / Silk Road / Roman Empire / India — cross-civilization connections: Buddhism spread along the Silk Road into Han China `[ec-023]`, Maurya contemporaneous with Persia; 3-tier sources incl. primary Ashoka edicts); **Journey → Guide → Source closed loop** (Guide renders Chinese relationship reasons — 18 frozen types zh templates via locale-aware selector; source chain clickable with `view_source` events; `open_entity` carries global id); **foundational exploration metrics** (`explorationMetrics.ts`: Depth / Package Coverage / Cross-Package Expansion / Guide Interaction; M71 telemetry `open_package` / `click_guide_next` / `view_source` / `complete_package` contract-reserved); **POC Ready** (validation materials: UX test kit / metrics baseline / demo script / institution value proposition / pilot acceptance criteria). Fixes from M71 findings: Guide reason i18n, Home navigation exits package context, "我的探索足迹" copy. Explicitly NOT: LLM runtime, free-form Q&A, generated facts, scoring / ranking / personalization. localStorage holds only anonymous local exploration-event cache — no accounts, no cloud sync, no user profiling. Pure frontend — zero backend / schema / AI Gateway / dependency change, backend diff = 0. Invariants: ENTITY_TYPES=8 / RELATIONSHIP_TYPES=18 untouched; runtime 0.13.0; no new dependency. freeze-check EXIT 0; tsc EXIT 0; 120 M71+M72 tests + 18 DiscoverPage regression green.
