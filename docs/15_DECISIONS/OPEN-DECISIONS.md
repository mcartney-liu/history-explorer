...
| 2026-08-15 | Phase 5 / 核心能力课题 | OD-12 过渡函数（Transition Function）课题 | describeTransition v1 三层已落地（①中文 claim 叙述 ②关系短句 ③留白，ConnectionCard 站间衔接 + EntityPage 入口桥共用）；课题文档 docs/product/TRANSITION_FUNCTION_SPEC.md；v2 候选=多跳路径桥/场景感知/双语策展（依赖 OD-11 数据）、v3=策展叙事弧/证据分级（P09）；图 grounded 禁编造 | 课题立项、分阶段演进（v1 已实现，v2/v3 排期研究） | v2/v3 数据前置依赖 OD-11 策展排期 | 分阶段排期立项 | OPEN | v1 已落地（2026-08-15）：describeTransition 共享函数，站间衔接 + 入口桥统一接入。v2 多跳路径桥 + 场景感知 + 双语策展、v3 策展叙事弧 + 证据分级，待排期。 |
